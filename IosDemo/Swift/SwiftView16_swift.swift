 ...
 *
 * 如果需要 swift 调用 oc
 * 1、需要创建一个名为 项目名称-Bridging-Header.h 的文件（桥接文件），并在 Build Settings 的 Objective-C Bridging Header 中配置好此文件的路径
 *    a) 本例中会有一个 IosDemo-Bridging-Header.h 文件，在 Build Settings 中的 Objective-C Bridging Header 配置的值是 IosDemo/Swift/IosDemo-Bridging-Header.h
 ...
 *
 * 注：
 * 如果你的 Build Settings 中没有 Objective-C Bridging Header 配置的话则可以文本方式编辑 project.pbxproj 文件，并在相关的位置增加 SWIFT_OBJC_BRIDGING_HEADER = "IosDemo/Swift/IosDemo-Bridging-Header.h";
 */
